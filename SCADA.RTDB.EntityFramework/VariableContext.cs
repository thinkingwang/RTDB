...
namespace SCADA.RTDB.EntityFramework
{
    public class VariableContext : DbContext, IVariableContext, IConvention
    {
        ...
        private void LoadVariable()
        {
            ...
            foreach (var analogVariable in AnalogSet)
            {
                if (!analogVariable.IsInitValueSaved)
                {
                    analogVariable.Value = analogVariable.InitValue;
                }
            }

            foreach (var digitalVariable in DigitalSet)
            {
                if (!digitalVariable.IsInitValueSaved)
                {
                    digitalVariable.Value = digitalVariable.InitValue;
                }
            }

            foreach (var textVariable in TextSet)
            {
                if (!textVariable.IsInitValueSaved)
                {
                    textVariable.Value = textVariable.InitValue;
                }
            }
        }
        ...
        public void ExitWithSaving()
        {
            foreach (var analogVariable in AnalogSet)
            {
                if (analogVariable.IsInitValueSaved)
                {
                    analogVariable.InitValue = analogVariable.Value;
                }
            }
            ...
            foreach (var digitalVariable in DigitalSet)
            {
                if (digitalVariable.IsInitValueSaved)
                {
                    digitalVariable.InitValue = digitalVariable.Value;
                }
            }
            ...
            foreach (var textVariable in TextSet)
            {
                if (textVariable.IsInitValueSaved)
                {
                    textVariable.InitValue = textVariable.Value;
                }
            }
            base.SaveChanges();
        }
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<DigitalVariable>().Ignore(p => p.UniqueId);
            modelBuilder.Entity<TextVariable>().Ignore(p => p.UniqueId);
            modelBuilder.Entity<AnalogVariable>().Ignore(p => p.UniqueId);
